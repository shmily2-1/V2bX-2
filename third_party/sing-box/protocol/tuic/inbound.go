...
func NewInbound(ctx context.Context, router adapter.Router, logger log.ContextLogger, tag string, options option.TUICInboundOptions) (adapter.Inbound, error) {
	...
	for index, user := range options.Users {
		inbound.uidToUuid[index] = user.Name
		inbound.uuidToUid[user.Name] = index
	}
	return inbound, nil
}
...
func (h *Inbound) NewConnectionEx(ctx context.Context, conn net.Conn, source M.Socksaddr, destination M.Socksaddr, onClose N.CloseHandlerFunc) {
	...
	userName, found := h.uidToUuid[userID]
	if !found {
		N.CloseOnHandshakeFailure(conn, onClose, E.New("user was removed"))
		return
	}
	if userName != "" {
	...
	onClose = N.AppendClose(onClose, func(error) { h.userconns.Delete(conn) })
